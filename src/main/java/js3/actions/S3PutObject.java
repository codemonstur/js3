...
public interface S3PutObject extends S3ClientConfiguration, S3Credentials {
    ...
    default void putObject(final String bucket, final String key, final byte[] data, final String contentType)
            throws IOException {
        putObject(bucket, key, data, new PutObjectOptions.Builder().withContentType(contentType).build());
    }

    default void putObject(final String bucket, final String key, final byte[] data,
